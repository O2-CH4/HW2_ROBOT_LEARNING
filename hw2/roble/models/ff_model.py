...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        # DONE
        # prediction = get numpy array of the predicted next-states (s_t+1)
        # Hint: `self(...)` returns a tuple, but you only need to use one of the outputs.
        obs = ptu.from_numpy(obs)
        acs = ptu.from_numpy(acs)
        new_data_statistics = {k: ptu.from_numpy(v) for k, v in data_statistics.items()}
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        """
        # DONE
        # target = ... compute the normalized target for the model.
